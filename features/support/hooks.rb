Before('~@reset_session') do
  ...
  maximize_window
  logger.info("Web App Version: #{app_version_info}")
...
After do |scenario|
  if TEST_CONFIG['js_log']
    logger.info('JS LOG ENTRIES:')
    js_errors.each { |entry| logger.info("#{entry.level}: #{entry.message}") }
  end
  ...
  if TEST_CONFIG && TEST_CONFIG['fail_fast']
    logger.info("'FAIL FAST' option is ON")
    # Tell Cucumber to quit after this scenario is done - if it failed
    if scenario.failed?
      logger.info("Terminating the run after the first failure for quicker feedback.\n" +
